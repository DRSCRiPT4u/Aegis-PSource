 ...
 * @author DrScript - timing pattern
 */ 
public class Fishing { 
    ...
    public static void doFishing(MapleCharacter chr, int baitLevel, double yearLikelihood, double timeLikelihood){
        ...
        if(chr.getWhenFishMax() != 0)
        {
            if(chr.getFishDaily() >= 3)
            {
                Date nowTime = new Date();
                Date fishTime = chr.getWhenFishDate();

                Date fishTimeDayLater = chr.getWhenFishDate();
                Calendar c = Calendar.getInstance();
                c.setTime(fishTimeDayLater);
                c.add(Calendar.DATE, 1);
                fishTimeDayLater = c.getTime();

                if(nowTime.before(fishTimeDayLater))
                {
                    chr.dropMessage(5, "You can only fish 3 times a day wait for tommarow to fish!");
                    return;
                }
                else
                {
        ...
        if (!hitFishingTime(chr, baitLevel, yearLikelihood, timeLikelihood)) {
            ...
            chr.setFishDaily(chr.getFishDaily() + 1);

            Date nowTime = new Date();
            DateFormat df = new SimpleDateFormat("yyyyMMdd");
            String s = df.format(nowTime);
            int output = Integer.valueOf(s);
            chr.setWhenFishMax(output);

            chr.dropMessage(5, "You have " + String.valueOf(3 - chr.getFishDaily()) + " attempts left!");
    ...
    public static int getRandomItem(){ 
        ...
        int[] uncommons = {1000025, 1002662, 1002812, 1002850, 1002537, 1002538, 1002539, 1002540, 1002541, 1012071, 1012072, 1012073, 1002881, 1002880, 1012072, 4020009, 2043220, 2043022, 2040543, 2044420, 2040943, 2043713, 2044220, 2044120, 2040429, 2043220, 2040943}; // filler' uptoo
        int[] rares = {4031284, 1002859, 1002553, 1003810, 1003811, 1003812, 1003813, 1022131, 1012540, 1012541, 1012542, 1012543, 1002762, 1002763, 1002764, 1002765, 1002766, 1002663, 1002788, 1002949, 2049100, 2340000, 2040822, 2040822, 2040822, 2040822}; // filler' uplast
