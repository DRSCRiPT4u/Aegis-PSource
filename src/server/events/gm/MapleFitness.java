...
public class MapleFitness {
           ...
           this.schedulemsg = TimerManager.getInstance().register(new Runnable() {
            ...
            if (chr.getMap().getId() >= 109040000 && chr.getMap().getId() <= 109040004) {
             if (getTimeLeft() > 9000 && getTimeLeft() < 11000) {
                 ...
                 chr.getClient().announce(MaplePacketCreator.serverNotice(0, "[MapleAegis Physical Fitness Test] consists of 4 stages, and if you happen to die during the game, you'll be eliminated from the game, so please be careful of that."));
             }
            } else {
             resetTimes();
            }
       ...
       // 14:30 [Notice][MapleAegis Physical Fitness Test] consists of 4 stages, and if you happen to die during the game, you'll be eliminated from the game, so please be careful of that.
