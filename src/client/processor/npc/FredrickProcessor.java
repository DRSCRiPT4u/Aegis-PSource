/*
    ...
    Copyleft (L) 2016 - 2019 DrScript (MapleAegis)
...
/**
 * @author DrScript - synchronization of Fredrick modules and operation results
 */
            ...
            try {
                ...
                try {
                    ...
                    if (deleteFredrickItems(chr.getId())) {
                        ...
                        if (merchant != null) merchant.clearItems();

                        items.stream().map(Pair::getLeft).forEach(item -> {
                            MapleInventoryManipulator.addFromDrop(chr.getClient(), item, false);
                            String itemName = MapleItemInformationProvider.getInstance().getName(item.getItemId());
                            SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy HH:mm");
                            FilePrinter.print(FilePrinter.FREDRICK + chr.getName() + ".txt", "[" + sdf.format(Calendar.getInstance().getTime()) + "] " + chr.getName() + " gained " + item.getQuantity() + " " + itemName + " (" + item.getItemId() + ")");
                        });
